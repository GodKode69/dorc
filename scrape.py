...
def run_pipeline(species, count):
    ...
    species_dir = NEW_DIR / species
    species_dir.mkdir(parents=True, exist_ok=True)

    existing_hashes = load_existing_hashes(NEW_DIR, UNCONFIRMED_DIR, config.DATA_DIR)
    ...
    while confirmed < count:
        ...
        for img_path in tqdm(images, desc="Classifying"):
            ...
            if pr_label == cl_label:
                dest_dir = NEW_DIR / cl_label
                dest_dir.mkdir(parents=True, exist_ok=True)
                dest = next_filename(dest_dir, cl_label)
                shutil.move(str(img_path), str(dest))
                stats["reclassified"] += 1
            else:
                dest_dir = UNCONFIRMED_DIR / cl_label
                dest_dir.mkdir(parents=True, exist_ok=True)
